...
void strdec_to_strbin(char num[5], char num_bin[33]) {
    ...
    int value = sign * abs_value; // considera o sinal
    unsigned int u = (unsigned int)value; // salva o numero no formato unsigned int
    for (int i = 0; i < 32; i++) {
        unsigned int bit = (u >> (31 - i)) & 1; // extrai o bit i
        num_bin[i] = bit ? '1' : '0';
    }
    num_bin[32] = '\0';
}
...
void and(char n1_bin[], char n2_bin[], char result_bin[]){
    for (int i = 0; i < 32; i++){
        // se ambos os bits forem 1, o resultado é 1
        // se um deles for 0, o resultado é 0
...
void nand(char n1_bin[], char n2_bin[], char result_bin[]){
    for (int i = 0; i < 32; i++){
        // se ambos os bits forem 1, o resultado é 0
        // se um deles for 0, o resultado é 1
        if (n1_bin[i] == '1' && n2_bin[i] == '1'){
            result_bin[i] = '0';
        } else {
            result_bin[i] = '1';
...
void or(char n1_bin[], char n2_bin[], char result_bin[]){
    for (int i = 0; i < 32; i++){
        // se um dos bits for 1, o resultado é 1
...
int main(){
    ...
    write(STDOUT_FD, val_final_hex, 9);
