# funcs.s - Implementacao de funcoes utilitarias e linked_list_search em RISC-V
...
# ------------------------------------------------------------------------------
# linked_list_search(Node *head_node, int val) -> a0 = head_node, a1 = val
# Procura 'val' na lista encadeada. Retorna o indice ou -1.
# Node: { int val1; int val2; Node *next; } (12 bytes)
# ------------------------------------------------------------------------------
.globl linked_list_search
linked_list_search:
    addi sp, sp, -16    # Aloca pilha
    sw ra, 12(sp)       # Salva ra
    sw s0, 8(sp)        # s0 = ponteiro do no atual
    sw s1, 4(sp)        # s1 = valor procurado
    sw s2, 0(sp)        # s2 = indice atual
    ...
    ll_loop:
        beqz s0, ll_not_found # Se s0 == NULL (0), nao encontrou

        lw t0, 0(s0)        # t0 = no->val1 (offset 0)
        lw t1, 4(s0)        # t1 = no->val2 (offset 4)
        add t2, t0, t1      # t2 = val1 + val2

        beq t2, s1, ll_found 

        lw s0, 8(s0)        # s0 = no->next (offset 8)
        addi s2, s2, 1      # indice = indice + 1
        j ll_loop           

    ll_found:
        mv a0, s2           # Retorna o indice (s2)
        j ll_end            # Pula para o fim
